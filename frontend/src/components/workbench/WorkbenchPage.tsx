                  ...
                  <div className="wizard-content">
                    <div className="supplement-section">
                      ...
                      {missingFields.length > 0 ? (
                        <div className="supplement-form grid gap-4 mt-6">
                          {missingFields.map((field) => {
                            // Extract current value from supplementaryInfo if it exists (Format: "Field: Value")
                            const regex = new RegExp(`^\s*${field}\s*:\s*(.*)'use client'
...
, 'm');
                            ...
                            return (
                              <div key={field} className="flex flex-col gap-2">
                                <label className="text-sm font-semibold text-slate-300 ml-1">{field}</label>
                                <input
                                  type="text"
                                  value={val}
                                  onChange={(e) => {
                                  ...
                                  className="w-full bg-slate-800/50 border border-slate-700/50 text-slate-100 placeholder-slate-500 rounded-lg px-4 py-3 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
